...
## Figura 1: Boxplot do Valor Total recebido por Rede (Brasil, 2023)
...
dados_esc %>%
  ggplot(aes(x = Rede, y = ValorTotal)) +
  geom_boxplot() +
  labs(x = "Rede",
       y = "Valor Total") +
  theme_minimal()
...
consorcios <- cnpjs %>% filter(Num_Registros > 2) 
...
## Figura 2: Boxplot do Valor Total recebido por Consórcio
...
# melt valor total e valor custeio em um única variavel


tabela_consorcios%>%
  ggplot(aes(y = ValorTotal)) +
  geom_boxplot() +
  labs(x = "Consórcio",
       y = "Valor Total") +
  theme_minimal()
