...
pacman::p_load(vroom,tidyverse, knitr, openxlsx, kableExtra,write)
setwd("C:/Users/User/Documents/GitHub/gradest-1/PDDE/entrega 7")

dados <- read.csv("base_completa_2024.csv")
dados_pagos <- dados %>% filter(Situacao_Pagamento == "Pagos")  %>% distinct(CO_ENTIDADE, .keep_all = TRUE)
dados_nao_pagos <- dados %>% filter(Situacao_Pagamento == "Nao Pagos")  %>% distinct(CO_ENTIDADE, .keep_all = TRUE)
...
# Visão Geral 
Os dados foram extraídos do Consulta Escola em 02 de julho de 2024.
...
dados$Diff_Alunos_Matriculados <- ifelse(dados$QT_MAT_BAS == dados$Quantidade.Alunos, "Igual", "Diferente")

dados$Diff_Alunos_Matriculados_Quant <- dados$QT_MAT_BAS - dados$Quantidade.Alunos
...
#rename fonte to Situacao_Pagamento

dados$Cadastro_UEx <- ifelse(dados$`CNPJ.Executora` == dados$`CNPJ.Município.SEDUC`, "Sem UEx", "Com UEx")

dados$CNPJ<- as.numeric(substr(dados$CNPJ.Executora, start=2,stop=10))

dados$Cadastro_UEx <- ifelse(is.na(dados$CNPJ), "Sem UEx", dados$Cadastro_UEx)

dados$Cadastro_UEx <- factor(dados$Cadastro_UEx, levels = c("Sem UEx", "Com UEx"))

dados$Alunos_Matriculados <- ifelse(as.numeric(dados$Quantidade.Alunos) > 50, "Maior que 50", "Menor ou igual a 50")
dados$Alunos_Matriculados <- factor(dados$Alunos_Matriculados, levels = c( "Menor ou igual a 50","Maior que 50"))

write.xlsx(dados,"base24.xlsx")
...
tab5 <- dados %>% 
        ...
        select(NO_UF, `pdde_pagos`,`pdde_naopagos`) %>%
...
tab14 <- dados %>% 
  ...
  group_by(NO_UF, TP_DEPENDENCIA) %>%
  mutate(TP_DEPENDENCIA = as.character(TP_DEPENDENCIA)) %>%
  summarise(n = n(), .groups = 'drop') %>%
  ungroup() %>%
  complete(NO_UF, TP_DEPENDENCIA = c("2", "3"), fill = list(n = 0)) %>%
  mutate(TP_DEPENDENCIA = as.numeric(TP_DEPENDENCIA)) %>%
  pivot_wider(names_from = TP_DEPENDENCIA, values_from = n, values_fill = list(n = 0)) %>%
  rename(`Estadual` = `2`, `Municipal` = `3`) %>%
  mutate(Total = rowSums(select(., -NO_UF))) %>%
  add_row(NO_UF = "Total", 
          Estadual = sum(.$Estadual, na.rm = TRUE), 
          Municipal = sum(.$Municipal, na.rm = TRUE), 
          Total = sum(.$Total, na.rm = TRUE)) %>%
  mutate(across(where(is.numeric), ~format(., decimal.mark = ",", big.mark = ".")))
...
tab18 <- escolas_somente_pagos %>%
        group_by(Destinação) %>% 
        summarise(n = n(), .groups = 'drop') %>% 
        #add_row(Destinação = "Total", n = sum(.$n, na.rm = TRUE)) %>%
...
tab20 <- dados_dup %>%
        ...
        summarise(n = n(), .groups = 'drop') %>% 
        add_row(Destinação.x = "Total",Destinação.y="", n = sum(.$n, na.rm = TRUE)) %>%
        mutate(n = format(n, big.mark = ".", decimal.mark = ","))%>%
...
* Soma dos totais das tabelas 6, 7 e 8.
```{r}
#| echo: false
#| warning: false
#| message: false
...
tab22 <- dados %>% 
        ...
        add_row(NO_UF = "Total", 
                ...
                n = sum(.$n, na.rm = TRUE)) %>%
        mutate(n = format(n, big.mark = ".", decimal.mark = ","))%>%
        rename("UF" = NO_UF, "Número de alunos" = alunos, "UEx" = UEx, "Número de escolas" = n)
#kable(tab22, align = "l", format="latex") %>%
 #       kable_styling(full_width = F, position = "left")%>%
 #       footnote(general = "Consulta Escola 2024; Censo Escolar 2023", general_title = "Fonte:")
```
...
\begin{tabular}{lll}
\hline
UF/Nº de alunos & Sem UEx & Com UEx\\
\hline
\multicolumn{3}{l}{\textbf{Distrito Federal}} \\
 Menos de 50 & 0 & 4\\
 Mais de 50 & 6 & 650\\
 Total
 \addlinespace
\multicolumn{3}{l}{\textbf{Goiás}}\\
Menos de 50 & 72 & 183\\
Mais de 50 & 60 & 3162\\
\addlinespace
\multicolumn{3}{l}{\textbf{Mato Grosso}} \\ 
Menos de 50 & 113 & 88\\
Mais de 50 & 65 & 1980\\
\addlinespace
\multicolumn{3}{l}{\textbf{Mato Grosso do Sul}} \\
Menos de 50 & 4 & 11\\
Mais de 50 & 11 & 1311\\
\hline
\multicolumn{3}{l}{\rule{0pt}{1em}\textit{Fonte:}}\\
\multicolumn{3}{l}{\rule{0pt}{1em}Consulta Escola 2024; Censo Escolar 2023}\\
\end{tabular}
...
#addWorksheet(wb, "UEx e N de alunos")
#writeData(wb, "UEx e N de alunos", tab22)
