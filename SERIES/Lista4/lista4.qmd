...
plot(phi, main = "", xlab = "Lag", ylab = "PACF", lwd = 3)
```
...
plot(phi, main = "", xlab = "Lag", ylab = "PACF")
```

10. Modelo de Previsão

### Utilizando a função `ar` e `predict`
...
energia$diff_modelo <- c(NA,diff(energia$predicted))

plot(energia$mes,energia$diff, type = "l", lwd = 2, 
    main = "Variação do consumo (KWh)", 
    xlab = TeX(r'($t$)'), 
    ylab = TeX(r'($Y_{t+1} - Y_t$)'))
lines(energia$mes,energia$diff_modelo, col = "red", lwd = 2)
```
...
```{r}
comp_modelos <- data.frame(Modelo = c("ar", "acf"), 
