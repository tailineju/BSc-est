...
df %>%
  ...
  ylim(0, 9000) +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/1_dist_lp.pdf", width = 8, height = 5, dpi = 300)
...
dfs %>%
  ...
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/2_qqplot_lp.pdf", width = 8, height = 5, dpi = 300)
...
df %>%
  ...
  ylim(100, 400) +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/3_boxplot_lp.pdf", width = 8, height = 5, dpi = 300)
...
df %>%
  ...
  ylim(0, 9000) +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/1_dist_mt.pdf", width = 8, height = 5, dpi = 300)
...
dfs %>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/2_qqplot_mt.pdf", width = 8, height = 5, dpi = 300)
...
df %>%
  ...
  ylim(100, 400) +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/3_boxplot_mt.pdf", width = 8, height = 5, dpi = 300)
...
dfcs %>%
  ...
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/13_hist_res_lp.pdf", width = 8, height = 5, dpi = 300)
...
dfcs %>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/14_qqplot_res_lp.pdf", width = 8, height = 5, dpi = 300)
...
dfc %>%
  ...
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/15_pred_res_lp.pdf", width = 8, height = 5, dpi = 300)

diag_lp <- performance::check_model(modelo6_lp, theme=theme_tcc, return=TRUE)
saveRDS(diag_lp,'T:/3. Resultados/1ª Extração de Resultados/MODELO_PERFORMANCE_LP.rds')
...
dfcs %>%
  ...
  labs(x = "Resíduos", y = "Frequência") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/13_hist_res_mt.pdf", width = 8, height = 5, dpi = 300)
...
dfcs %>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/14_qqplot_res_mt.pdf", width = 8, height = 5, dpi = 300)
...
dfc %>%
  ...
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/15_pred_res_mt.pdf", width = 8, height = 5, dpi = 300)

# Escolas
residuos_mt_esc <- ranef(modelo6_mt)$ID_ESCOLA
residuos_lp_esc <- ranef(modelo6_lp)$ID_ESCOLA
...
residuos_mt_esc%>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/16_qqplot_res_inter_mt.pdf", width = 8, height = 5, dpi = 300)

residuos_mt_esc%>%
  ggplot(aes(sample = VL_INSE_ALUNO)) +
  stat_qq() +
  stat_qq_line()+
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/17_qqplot_res_inse_mt.pdf", width = 8, height = 5, dpi = 300)

residuos_mt_esc%>%
  ggplot(aes(sample = GRUPO_RACIAL)) +
  stat_qq() +
  stat_qq_line()+
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/18_qqplot_res_gpr_mt.pdf", width = 8, height = 5, dpi = 300)

residuos_mt_esc%>%
  ggplot(aes(sample = REPROVACAO)) +
  stat_qq() +
  stat_qq_line()+
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/19_qqplot_res_rep_mt.pdf", width = 8, height = 5, dpi = 300)

# Homocedasticidade

# Adicionando coluna com os valores ajustados por aluno

dfc$fitted_values_mt <- NA
dfc$fitted_values_mt[as.numeric(names(fitted(modelo6_mt)))] <- fitted(modelo6_mt)

# Agregando os valores ajustados por escola

fitted_escola <- dfc %>% 
  group_by(ID_ESCOLA) %>% 
  summarise(media_fitted = mean(fitted_values_mt, na.rm=T))

# Escolas que aparecem em ranef()

escolas_com_efeitos <- rownames(residuos_mt_esc)

# Filtrando apenas as escolas que aparecem em ranef()

fitted_escola_filt <- fitted_escola %>% 
  filter(ID_ESCOLA %in% escolas_com_efeitos)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_mt_esc$`(Intercept)`)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/20_pred_res_inter_mt.pdf", width = 8, height = 5, dpi = 300)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_mt_esc$VL_INSE_ALUNO)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/21_pred_res_inse_mt.pdf", width = 8, height = 5, dpi = 300)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_mt_esc$REPROVACAO)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/22_pred_res_rep_mt.pdf", width = 8, height = 5, dpi = 300)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_mt_esc$GRUPO_RACIAL)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/23_pred_res_gpr_mt.pdf", width = 8, height = 5, dpi = 300)

diag_mt <- performance::check_model(modelo6_mt, theme=theme_tcc, return=TRUE)
saveRDS(diag_mt,'T:/3. Resultados/1ª Extração de Resultados/MODELO_PERFORMANCE_MT.rds')
...
residuos_lp_esc%>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/16_qqplot_res_inter_lp.pdf", width = 8, height = 5, dpi = 300)
...
residuos_lp_esc%>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/17_qqplot_res_inse_lp.pdf", width = 8, height = 5, dpi = 300)
...
residuos_lp_esc%>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/18_qqplot_res_gpr_lp.pdf", width = 8, height = 5, dpi = 300)
...
residuos_lp_esc%>%
  ...
  labs(x = "Quantis teóricos", y = "Quantis amostrais") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/19_qqplot_res_rep_lp.pdf", width = 8, height = 5, dpi = 300)

# Homocedasticidade

# Adicionando coluna com os valores ajustados por aluno

dfc$fitted_values_lp <- NA
dfc$fitted_values_lp[as.numeric(names(fitted(modelo6_lp)))] <- fitted(modelo6_lp)

# Agregando os valores ajustados por escola

fitted_escola <- dfc %>% 
  group_by(ID_ESCOLA) %>% 
  summarise(media_fitted = mean(fitted_values_lp, na.rm=T))

# Escolas que aparecem em ranef()

escolas_com_efeitos <- rownames(residuos_lp_esc)
...
fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_lp_esc$`(Intercept)`)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/20_pred_res_inter_lp.pdf", width = 8, height = 5, dpi = 300)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_lp_esc$VL_INSE_ALUNO)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/21_pred_res_inse_lp.pdf", width = 8, height = 5, dpi = 300)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_lp_esc$REPROVACAO)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/22_pred_res_rep_lp.pdf", width = 8, height = 5, dpi = 300)

fitted_escola_filt %>%
  ggplot(aes(x = media_fitted, y = residuos_lp_esc$GRUPO_RACIAL)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predições", y = "Resíduos") +
  theme_tcc()
ggsave("T:/3. Resultados/1ª Extração de Resultados/IMG/23_pred_res_gpr_lp.pdf", width = 8, height = 5, dpi = 300)
...
cat('\n # Medidas Resumo - INSE Aluno \n')
summary(df$VL_INSE_ALUNO)
cat('\n',paste0('SD: ',sd(df$VL_INSE_ALUNO)), '\n')
...
cat('\n # Tabela de Frequências: Escolaridade da mãe \n')
df %>%
  count(ESCOLARIDADE_MAE) %>%
  mutate(prop=round(n/sum(n)*100,2)) %>%
  select(ESCOLARIDADE_MAE,prop)%>%
  kable(format.args = list(decimal.mark=','))

cat('\n # Tabela de Frequências: NSE Escola \n')
df %>%
  count(NSE_ESCOLA) %>%
  mutate(prop=round(n/sum(n)*100,2)) %>%
  select(NSE_ESCOLA,prop)%>%
  kable(format.args = list(decimal.mark=','))

cat('\n # Tabela de Frequências: Grupo Racial \n')
df %>%
  mutate(GRUPO_RACIAL = ifelse(GRUPO_RACIAL == 1, "Brancos e Amarelos", "PPI")) %>%
  count(GRUPO_RACIAL) %>%
  mutate(prop=round(n/sum(n)*100,2)) %>%
  select(GRUPO_RACIAL,prop)%>%
  kable(format.args = list(decimal.mark=','))

cat('\n # Tabela de Frequências: Reprovação \n')
df %>%
  mutate(REPROVACAO = ifelse(REPROVACAO == 0, "Nenhuma", ifelse(REPROVACAO == 1, "Uma", "Duas ou mais"))) %>%
  mutate(REPROVACAO = factor(REPROVACAO, levels = c("Nenhuma", "Uma", "Duas ou mais"))) %>%
  count(REPROVACAO) %>%
  mutate(prop=round(n/sum(n)*100,2)) %>%
  select(REPROVACAO,prop)%>%
  kable(format.args = list(decimal.mark=','))

cat('\n # Tabela de Frequências: Localização da Escola \n')
df %>%
  mutate(ID_LOCALIZACAO = ifelse(ID_LOCALIZACAO == 1, "Urbana", "Rural")) %>%
  mutate(ID_LOCALIZACAO = factor(ID_LOCALIZACAO, levels = c("Urbana", "Rural"))) %>%
...
cat('\n # Tabela de Frequências: Área da Escola \n')
df %>%
  mutate(ID_AREA = ifelse(ID_AREA == 1, "Capital", "Interior")) %>%
  count(ID_AREA) %>%
  mutate(prop=round(n/sum(n)*100,2)) %>%
