...
diag <- anti_join(df_aluno,df_escola, by = c("ID_ESCOLA", "ID_REGIAO","ID_SAEB", "ID_UF","ID_MUNICIPIO","ID_AREA","IN_PUBLICA", "ID_LOCALIZACAO"))

df_escola <- df_escola %>% filter(!ID_ESCOLA %in% diag$ID_ESCOLA)
...
df <- vroom("dados_tcc3.csv", locale = locale(encoding = "UTF-8"), delim = ",")
...
df %>% 
    ...
    kable(booktabs = TRUE, caption = "Frequência do Nível de Escolaridade dos Responsáveis", format = "latex") %>%
    kable_styling()
