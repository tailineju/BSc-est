...
df_aluno <- df_aluno %>% filter(IN_PREENCHIMENTO_LP ==1) #1.943.252
df_aluno <- df_aluno %>% filter(IN_PREENCHIMENTO_MT ==1) #1.942.375
...
df_escola <- df_escola %>% 
  filter(ID_ESCOLA %in% unique(df$ID_ESCOLA))

df %>% summarise(n_distinct(ID_ESCOLA)) %>% pull()

df <- df%>% filter(ID_REGIAO == 5)

df <- df %>% 
  mutate(ID_ESCOLA = as.factor(ID_ESCOLA))

dim(df)
write.csv(df,"dadostcc_2407.csv")
```
...
df <- vroom("dadostcc_3.csv", locale = locale(encoding = "UTF-8"), delim = ",")
