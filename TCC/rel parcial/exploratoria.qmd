...
df %>%
    ...
    mutate(TX_RESP_Q002 = factor(TX_RESP_Q002, levels = c("Branca", "Preta", "Parda", "Amarela", "Indígena", "Não declarado")))%>%
...
df %>%
    ...
    mutate(TX_RESP_Q004 = case_when(
      TX_RESP_Q004 == "A" ~ "< Fundamental",
      TX_RESP_Q004 == "B" ~ "Fundamental Incomp",
      TX_RESP_Q004 == "C" ~ "Fundamental",
      TX_RESP_Q004 == "D" ~ "Médio",
      TX_RESP_Q004 == "E" ~ "Superior",
      TX_RESP_Q004 == "F" ~ "Não sei",
      TRUE ~ TX_RESP_Q004
    )) %>%
    mutate(TX_RESP_Q004 = factor(TX_RESP_Q004, levels = c("Não sei","< Fundamental", "Fundamental Incomp", "Fundamental", "Médio", "Superior"))) %>%
...
df %>%
  ...
  geom_boxplot(fill = "white") +
  labs(x = "Tempo para chegar a escola", y = "Língua Portuguesa") +
  ylim(100,400)+
  theme_tcc()
...
df %>%
    ...
    mutate(TX_RESP_Q018C = factor(TX_RESP_Q018C, levels = c("Nunca ou quase nunca", "De vez em quando", "Sempre ou quase sempre"))) %>%
